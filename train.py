...
import prune_utils # pylint: disable=wrong-import-position, wrong-import-order
            ...
            if teacher_model is not None:
                with torch.no_grad():
                    teacher_logits = teacher_model(
                        input_ids=subbatch_input_ids).logits
...
def main(args):
    ...
    model = tf.AutoModelForCausalLM.from_pretrained(args.model_card).to(device)
    if device.type != "cpu":
    ...
    if args.do_prune:
        iterative_pruner = prune_utils.Pruner(args.prune_recipe)
        ...
        if iter_idx and iter_idx != prev_iter_idx and iter_idx % LOG_FREQUENCY == 0:
            ...
            if iterative_pruner.is_it_time_to_prune():
                ...
                save_path = os.path.join(
                    args.save_dir, f"ckpt_{(iterative_pruner.times_pruned)}.pt")
                times["Save"] += save_checkpoint(
                    iter_idx, model, optimizer, save_path, device)
                # Prune the model and evaluate before and after pruning.
                evaluate_model(
                    model, eval_dataloader, 2*args.subbatch_size, loss_fn_eval, device, args.wandb)
                iterative_pruner.prune_model(
                    model=model, dataloader=train_dataiterator, optimizer=optimizer)
                evaluate_model(
                    model, eval_dataloader, 2*args.subbatch_size, loss_fn_eval, device, args.wandb)
            assert not iterative_pruner.should_prune()

        if iter_idx and iter_idx != prev_iter_idx and iter_idx % MODEL_CKPT_FREQUENCY == 0:
            # save_iter_idx = 0 # iter_idx
            save_path = os.path.join(
                args.save_dir, f"ckpt_{(iterative_pruner.times_pruned)}.pt")
            iterative_pruner.remove_prune(model=model)
            times["Save"] += save_checkpoint(iter_idx, model, optimizer, save_path, device)
            iterative_pruner.init_prune(model=model)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dummy', dest='dummy', action='store_true',
                        help="Run model on small sample for debugging.")
    parser.add_argument('--model_card', type=str, help='Path to the model', required=True)
    parser.add_argument('--train_data_path', type=str, help='Path to the model', required=True)
    parser.add_argument('--eval_data_path', type=str, help='Path to the model', required=True)

    parser.add_argument('--batch_size', type=int, help='Batch size.', default=16)
    parser.add_argument('--subbatch_size', type=int, help='Splits batch for Grad Accum.',
                        default=4)
    parser.add_argument('--max_len', type=int, help='Maximum sequence length.', default=256)
    parser.add_argument('--lr', type=float, help='Initial learning rate.', default=1e-5)

    parser.add_argument('--save_dir', type=str, help='Model Save path', default="savefolder")
    parser.add_argument('--load_dir', type=str, help='Model Load path', default="")
    parser.add_argument('--wandb', dest='wandb', action='store_true', help='Whether to log wandb.')

    parser.add_argument('--do_prune', dest='do_prune', action='store_true',
                        help="Whether to prune.")
    parser.add_argument('--prune_recipe_path', dest='prune_recipe_path', type=str,
                        default=None, help="Path to prune recipe.")

    parser.add_argument('--do_knowledge_distillation', dest='do_knowledge_distillation',
                        action='store_true', help="Whether to do knowledge distillation.")
    parser.add_argument('--kd_weight', type=float, help='Knowledge distillation weight.',
                        default=0.0)
    parser.add_argument('--kd_temperature', type=float, help='Knowledge distillation temperature.',
                        default=0.0)
    parser.add_argument('--kd_teacher_model_path', type=str, help='Teacher model path.',
                        default=None)

    parser_args = parser.parse_args() # pylint: disable=invalid-name

    # Sanity checks
    assert parser_args.batch_size % parser_args.subbatch_size == 0, \
        "Batch size must be divisible by num_subbatch_for_accum"

    # Sanity check for pruning params
    if parser_args.do_prune:
        with open(parser_args.prune_recipe_path, "r") as f: # pylint: disable=unspecified-encoding
            parser_args.prune_recipe = edict(yaml.safe_load(f))
        print("Prune recipe:", parser_args.prune_recipe)

    # Sanity check for knowledge distillation params
    if parser_args.do_knowledge_distillation:
        assert parser_args.kd_weight > 0.0, "KD weight must be > 0.0"
        assert parser_args.kd_temperature > 0.0, "KD temperature must be > 0.0"

    # Setup if dummy run.
    if parser_args.dummy:
        LOG_FREQUENCY = 5
        MODEL_CKPT_FREQUENCY = 10
